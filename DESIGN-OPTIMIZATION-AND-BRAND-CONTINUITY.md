...
- The project-owned page archetype, content rhythm, visual identity, and distinctive selectors for every route family.
...
### Give route families a reason to differ

A multi-page site should not repeat one page composition and replace only the nouns. Define the reader purpose, page archetype, content rhythm, visual identity, and media role for each route family. Routes in one article, product-detail, or legal-document family may intentionally share a composition. Distinct route families must show measured differences that support their purpose.

Run the mandatory [Interface Quality and Page Differentiation Gate](INTERFACE-QUALITY-AND-PAGE-DIFFERENTIATION.md). Do not create arbitrary class names or decorative changes to satisfy it. Use differences in structure, layout, palette, typography, or media only when they improve comprehension and preserve the brand.
...
- Measured clearance between important actions and nearby separators, status bands, or structural regions.
- Route-family differentiation in structure, layout, palette, typography, and media.
...
- Interface-quality report, route-family contracts, clearance results, hero and header measurements, and page-differentiation comparisons.
...
- The interface-quality report fails on any required route, viewport, browser, clearance contract, hero or header limit, or route-family comparison.
...
- Every indexable route is covered by the interface-quality contract, and distinct route families pass the measured differentiation threshold.

...
export function wordShingles(value, size = 2) {
  const normalizedWords = words(value).map((word) => word.toLowerCase());
  if (normalizedWords.length < size) return new Set(normalizedWords);
  return new Set(normalizedWords.slice(0, normalizedWords.length - size + 1).map((_, index) => normalizedWords.slice(index, index + size).join(" ")));
}

export function jaccardSimilarity(first, second) {
  const firstSet = first instanceof Set ? first : new Set(first);
  const secondSet = second instanceof Set ? second : new Set(second);
  if (firstSet.size === 0 && secondSet.size === 0) return 1;
  const intersection = [...firstSet].filter((value) => secondSet.has(value)).length;
  const union = new Set([...firstSet, ...secondSet]).size;
  return union === 0 ? 0 : Number((intersection / union).toFixed(4));
}

export function contentSimilarity(first, second, shingleSize = 2) {
  return jaccardSimilarity(wordShingles(first, shingleSize), wordShingles(second, shingleSize));
}

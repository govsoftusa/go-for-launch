...
verify every indexable route passes interface geometry, reviewed clearance, header and hero proportions, and route-family differentiation in Chromium and WebKit
...
Before committing, packaging, or publishing Go for Launch, follow [CASE-STUDY-NORMALIZATION.md](CASE-STUDY-NORMALIZATION.md) and run `npm run case-studies:verify`. Every case study filename and complete document must omit client, organization, person, domain, repository, account, infrastructure-resource, release-candidate, and exact correlatable artifact identifiers. Each case study must contain the required full-file review marker.

A normalization finding blocks commit and release. The automated verifier is a backstop, not permission to retain an identity it cannot recognize. Do not add a client value to an allowlist or publish a release while known identifying content remains in the current tree.
...
Run [Interface Quality and Page Differentiation](INTERFACE-QUALITY-AND-PAGE-DIFFERENTIATION.md) against the exact built candidate. Every indexable route must have a route-family contract and visible page-archetype marker. The verifier must cover the reviewed expanded, compact desktop, tablet, mobile, and 320 CSS pixel widths in Chromium and WebKit. Control overlap, clipped controls, horizontal overflow, failed project clearance rules, failed reviewed header or hero proportions, missing route-specific elements, or insufficient measured differentiation between distinct route families blocks production.

Automated route-family comparison measures structure, layout, palette, typography, and media. It does not authorize arbitrary visual differences. Review the rendered routes at native size and confirm that each difference supports the page purpose while preserving brand continuity.
...
- The interface-quality report is missing or fails on control overlap, clipping, viewport bounds, clearance, header or hero proportions, route coverage, archetype markers, or route-family differentiation.
...
- Source revision.
- Build result.
- Interface-quality report path, covered routes, browser and viewport matrix, clearance results, header and hero measurements, page-family differentiation, and reviewed target-size exceptions.

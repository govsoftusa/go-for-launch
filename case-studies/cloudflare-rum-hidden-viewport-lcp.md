...
This case study records a July 2026 production performance incident on an Astro site hosted by Cloudflare. It is historical evidence for the reusable rules in [Cloudflare Production Observability](../CLOUDFLARE-OBSERVABILITY.md).
...
- `/images/Frame-5_1.webp` as the LCP asset path.
- A homepage desktop LCP P75 of 6,240 ms in the selected historical window.
...
An approved Global API Key stored in the secret manager was then tested with Cloudflare's documented key headers. It could read the zone dataset. The selected 24-hour window contained 25,908 requests, no `5xx` responses, and 9,931 `4xx` responses. The leading `404` paths were automated WordPress and PHP probes rather than missing site navigation. This evidence changed the reusable default so raw `4xx` rate remains report-only until a project reviews scan traffic and selects an enforceable limit.

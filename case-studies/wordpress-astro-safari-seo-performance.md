...
This case study is maintained under `case-studies/` and indexed by the repository `README.md`. Use the shared porting playbook for general guidance and this document for site-specific evidence. The application source remains a separate project checkout, so confirm its current location before editing, testing, or deploying application code.

...
6. Copy and complete `templates/interface-quality.config.mjs` when the project has public routes. Keep route families, archetype names, selectors, header and hero limits, and clearance thresholds in the target project.
7. Put custom behavior in a project-owned wrapper, adapter, validator, or generator when the shared behavior is not sufficient.
8. Wire the project-owned command into the normal build or test chain before the shared release gates that depend on it.
9. Prove the extension runs through the accepted project command. A standalone script that the normal build can skip is not integrated.
10. Run the Go for Launch test suite after shared documentation or code changes. Run the target project's full test suite after local extension changes.
11. Record the toolkit revision, project test result, and any renewed human approval in the project's release evidence.

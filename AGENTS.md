...
- Read `CASE-STUDY-NORMALIZATION.md` before creating or changing a case study. Case-study filenames and contents must not identify clients, organizations, people, domains, repositories, accounts, infrastructure resources, release candidates, or exact correlatable artifacts. Use neutral descriptions and approved example hosts.
- Run `npm run case-studies:verify` before committing, packaging, or releasing. Every case study must carry the required full-file review marker. A normalization finding blocks commit and release, and client identifiers must never be added to the verifier allowlist.
...
- Before implementation begins, read `EXECUTION-CONTROL.md`, copy `templates/execution-control-record.md` into the target repository, and record the task envelope, completion conditions, scope exclusions, control thresholds, rollback method, and initial candidate state.
- Treat execution time and attempt limits as pause conditions, never release waivers. Reaching a control threshold leaves every failed mandatory gate failed and blocks production until an explicit next action is selected.
- Read `REQUEST-BUDGET-AND-LARGE-SITE-VERIFICATION.md` before any archive-wide or public-host verification. Record estimated and maximum external requests and transfer bytes before starting. Stop before the next request when either maximum is reached.
- A local browser gate must block and count external requests by default. Do not allow a local route sweep to request production, CDN, analytics, anti-bot, font, or media resources.
- Run `scripts/verify-execution-control.mjs` at phase checkpoints and before production readiness. A missing task envelope, unreviewed raised threshold, active blocker at its control limit, missing exact-candidate gate declaration, or PageSpeed score below 100 blocks the process claim.
- After two unsuccessful attempts or ninety minutes of active work on one blocker, stop speculative changes, record the evidence and attempted fixes, and mark the task blocked unless a project owner explicitly approves a supported bounded continuation.
- Use targeted tests during development, but run the complete mandatory release suite against the exact frozen candidate after the final change. Any later source, content, dependency, configuration, infrastructure, or artifact change creates a new candidate and requires the complete mandatory suite again before production.
- Classify every proposed production mutation before acting. Use the editorial publishing lane only when the change is limited to CMS records and media, does not alter application source, dependencies, configuration, infrastructure, routing, cache implementation, templates, or built artifacts, and can be rolled back through the CMS.
- Read `EDITORIAL-PUBLISHING-AND-DYNAMIC-CONTENT.md` before publishing or correcting CMS content. Copy `templates/editorial-publish-record.md` and `templates/editorial-publish.config.mjs` into the target project, record the content delta and affected route graph, and run `scripts/verify-editorial-publish.mjs` after the mutation.
- Do not run an Astro build, create a new application candidate, upload a build artifact, or start the full application release suite for a qualifying editorial publish. Validate the changed entry and its dependent public routes through targeted checks against the current verified application candidate.
- Keep a normal editorial publish at or below 200 external requests and 250,000,000 transfer bytes. Record the estimate and observed total. An overrun blocks the publishing record and requires a redesigned targeted check.
- Escalate immediately to the application release lane when an editorial task requires a renderer, component, schema, dependency, configuration, routing, cache-code, security-boundary, or infrastructure change. Split hybrid work into a reversible CMS correction and a separately authorized application release.
- Editorial publishing never waives or replaces the mandatory application release gates. Every application release still requires the exact frozen candidate, complete mobile and desktop coverage, Playwright WebKit, native iOS Safari, and PageSpeed scores of 100 for Performance, Accessibility, Best Practices, and SEO.
- Use a supported CMS UI or API for normal publication. Direct database writes are break-glass operations that require explicit approval, a backup, a documented rollback, and post-write parity verification.
- Read `CMS-AUTHENTICATION-SESSION-GATE.md` for every authenticated CMS or administrative console. Copy `templates/cms-authentication-session-record.md` into the target project and require credential assertion, immediate authenticated identity, collection lists, existing editors, disposable draft create, edit, reload, and cleanup against the compiled private candidate.
- Do not accept a login screen, successful passkey ceremony, redirect, cookie, admin shell, or 3xx API response as proof of a usable CMS session. Eventually consistent cache storage cannot be the sole authentication session source of truth unless the framework has a reviewed handoff that survives an immediate read miss.
- Purge only the changed content record and its known dependent route graph. Treat HTML cache, Worker Cache API, CDN cache, object cache, and CMS query cache as distinct layers, and record which layers were invalidated.
- A successful source-media request does not prove the publication media object exists. Reconcile media identifiers, object keys, dimensions, content type, and delivery URL before publishing.
- Do not automatically replace featured images across an archive. Produce a review queue when a selected image is undersized, duplicated, badly cropped, or disconnected from the article, then require editorial selection from suitable source images.
- Record symptoms, root cause, failed experiments, rollback, targeted checks, and performance evidence while publishing. A content correction that reveals an application defect does not authorize an application change.
- Classify newly discovered work as required outcome, release blocker, recommended follow-up, or unrelated. Do not expand the active task for recommended or unrelated work without explicit scope approval.
...
- Compare route openings, closings, and full copy across content families. Repeated generic arguments and calls to action must be rewritten or covered by a specific reviewed exception.
...
- Read `INTERFACE-QUALITY-AND-PAGE-DIFFERENTIATION.md`, configure every indexable route, and run `scripts/verify-interface-quality.mjs` against the exact production build. Control overlap, clipping, horizontal overflow, failed clearance contracts, failed reviewed hero or header proportions, missing route-family contracts, and insufficient measured differentiation block production.
...
- Run the case-study normalization verifier before packaging or publishing Go for Launch. A missing marker, identifying filename or content, personal path, nonapproved host, personal contact value, or correlatable infrastructure identifier blocks the toolkit release.
...
- Generate a passing machine-readable interface-quality report across expanded,
  compact desktop, tablet, mobile, and 320 CSS pixel widths. Browser-test every
  reviewed template and meaningful rendering variant. Preserve an exhaustive
  static inventory that maps every indexable route to a tested coverage class.
...
- Preserve the machine-readable interface-quality report and its route, viewport, browser, clearance, hero, header, and page-differentiation results with release evidence.

...
3. Build the website.
4. Run the verifier once to obtain each page's content hash and automated findings.
5. Rewrite the content until the automated errors are resolved.
6. Review the rendered page while adopting the senior psychology professor perspective.
7. Copy [`templates/content-quality.reviews.json`](templates/content-quality.reviews.json), record the exact hash, and complete every approval field with a specific note.
8. Run the verifier again. A missing or stale review blocks the build.
9. Rebuild and repeat the review whenever public content changes.
